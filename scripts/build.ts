...
await bundle({ entrypoints: ['src/backend/index.ts'], compile: { outfile: 'dist/backend/moki-runtime' }, target: 'bun' });
...
if (process.platform === 'darwin') {
  const signing = Bun.spawn(['codesign', '--force', '--sign', '-', 'dist/backend/moki-runtime'], { stdout: 'inherit', stderr: 'inherit' });
